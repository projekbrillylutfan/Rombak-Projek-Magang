...
describe("POST /api/users/update", () => {
  ...
  afterEach(async () => {
    await UserTest.delete();
    await UserTest.deleteUpdate();
  });

  it("should return 200 if update user success", async () => {
    const response = await supertest(web)
    ...
    expect(response.body.data.nama).toBe("test update");
    expect(response.body.data.jabatan).toBe("test update");
  });
